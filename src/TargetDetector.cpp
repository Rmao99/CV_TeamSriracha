...
// 2000 should be changed after testing
    ...
    if ( ( (approxContour.size() == corners) /*|| ( (approxContour.size() == corners) && (corners == 4)*/ /*&& (contours[i][1].y >= 200)*//* ) ) && (cv::contourArea(approxContour, false) > 400)*/)) {
		if(corners == 4)
		{
		    if((contours[i][1].y >= 200) && (cv::contourArea(approxContour, false) > 400)
         	{
         	    double maxCosine = 0;
          		for (int j = 2; j<=4; j++) 
			    {
		...
		{
		    if(cv::contourArea(approxContour, false) > 400)
		    {
			    cv::drawContours(img,contours,i,Scalar(0,255,0),5);
			    std::cout <<"y-coordinate of first cross contour: " << contours[i][1].y << "\n";
			    return approxContour
		    }
	    }
	}
